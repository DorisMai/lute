...
def tjump_analysis_summary(
    ...
    filter_mask_keys.append("Scaled_Smoothed_AirSubtracted")
    # Create panel tabs for intermediate plots loaded from png files generated by
    # tjump analysis script. Assume the png files are named as the filter mask keys.
    tabs_list = []
    ...
    for filter_mask_key in filter_mask_keys:
        ...
        if os.path.exists(png_file):
            tabs_list.append((filter_mask_key, pn.pane.PNG(png_file)))
        else:
    ...
    if os.path.exists(summary_png_file):
        summary_plot = pn.pane.PNG(summary_png_file)
    else:
