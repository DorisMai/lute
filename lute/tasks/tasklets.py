...
def tjump_analysis_summary(
    ...
    Returns:
        Tuple containing:
            ...
            - Optional[ElogSummaryPlots]: Plot object for elog display, or None if failed
    """
    import panel as pn
    pn.extension()
    col_widths = [700, 900]
    # Create panel tabs for intermediate plots loaded from png files generated by tjump analysis scipt.
    # Hard coding figure/filter names for now as they are not consistent.
    filter_mask_keys = ["scattering_falloff_filter_pass",
    ...
    for filter_mask_key in filter_mask_keys:
        ...
        if os.path.exists(png_file):
            png_pane = pn.pane.PNG(png_file, width=col_widths[0])
            tabs_list.append((filter_mask_key, png_pane))
        else:
            logger.warning(f"PNG file {png_file} not found")
    intermediate_tabs = pn.Tabs(*tabs_list) if tabs_list else pn.pane.Markdown("No intermediate plots found")
    ...
    if os.path.exists(summary_png_file):
        summary_plot = pn.pane.PNG(summary_png_file, name="summary_plot", width=col_widths[1])
    else:
        logger.warning(f"Summary PNG file {summary_png_file} not found")
        summary_plot = pn.pane.Markdown("Summary plot not found")
    
    # Load intermediate and summary plots side by side
